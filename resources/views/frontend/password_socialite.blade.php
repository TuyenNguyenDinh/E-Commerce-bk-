...
<div class="container">
    <div class="section">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">{{ __('content.Please create password before redirect')}}</div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('postPasswrSocial') }}">
                            @csrf
                            <div class="form-group row">
                                <label for="email" class="col-md-4 col-form-label text-md-right">{{ __('content.Email address')}}</label>
                                <div class="col-md-6">
                                    <input id="email_reset" type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{$email}}"  autocomplete="email" autofocus readonly>
                                    @error('email')
